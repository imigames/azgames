...
import { b as adminCreateGenerationLog } from './ai-providers_Dsks5SxJ.mjs';
import { g as generateAiText } from './ai-client_GU1hroY7.mjs';
import { b as sanitizeGameArticleHtml, h as htmlToPlainText } from './html-sanitize_DC9IPhPx.mjs';
import { a as adminGetPromptTemplateByKey, r as renderPromptTemplate } from './prompt-templates_CUEdmw4k.mjs';
...
const isConfigured = () => Boolean(supabaseAdmin);
...
const getFriendlyAiErrorMessage = (message = "") => {
  ...
  const mentionsTokens = normalized.includes("max_tokens") || normalized.includes("max tokens") || normalized.includes("token");
  if (normalized.includes("fewer max_tokens") || normalized.includes("insufficient credits") || mentionsCredits && mentionsTokens) {
...
const stripMarkdownFences = (value) => value.trim().replace(/^```(?:html|markdown|md|text)?\s*/i, "").replace(/\s*```$/i, "").trim();
const inlineMarkdownToHtml = (value) => escapeHtml(value).replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>").replace(/\*([^*]+)\*/g, "<em>$1</em>");
const markdownToSafeHtml = (value) => {
...
const normalizeGeneratedArticle = (value) => {
  const stripped = stripMarkdownFences(value);
  const html = /<\s*(h1|h2|h3|p|ul|ol|li|blockquote|strong|em|a|br)\b/i.test(stripped) ? stripped : markdownToSafeHtml(stripped);
  return sanitizeGameArticleHtml(html).trim();
};
const normalizeGeneratedMetaDescription = (value) => {
  const plainText = htmlToPlainText(stripMarkdownFences(value)).replace(/^["']|["']$/g, "").replace(/\s+/g, " ").trim();
...
const upsertSettings = async (values) => {
  if (!supabaseAdmin) {
...
const getCategoryName = async (categorySlug) => {
  if (!categorySlug || !supabaseAdmin) {
    return categorySlug ?? "";
  }
  const { data } = await supabaseAdmin.from("categories").select("slug,name").eq("slug", categorySlug).maybeSingle();
  return data?.name ?? categorySlug;
};
const applyContentStatusFilter = (query, contentStatus = "all") => {
  if (contentStatus === "ready") {
    return query.not("title", "is", null).neq("title", "").not("slug", "is", null).neq("slug", "").not("iframe_url", "is", null).neq("iframe_url", "").not("thumbnail", "is", null).neq("thumbnail", "").not("category_slug", "is", null).neq("category_slug", "");
  }
...
const countInactiveGames = async (contentStatus = "all") => {
  if (!supabaseAdmin) {
    return 0;
  }
  let query = supabaseAdmin.from("games").select("id", { count: "exact", head: true }).eq("is_active", false);
...
const hasDuplicateSlug = async (game) => {
  if (!supabaseAdmin || !game.slug.trim()) {
    return false;
  }
  const { data, error } = await supabaseAdmin.from("games").select("id").eq("slug", game.slug).neq("id", game.id).limit(1);
  if (error) {
    throw new Error(error.message);
  }
  return Boolean(data?.length);
};
const getPublishBlockReason = async (game, {
  requireArticle,
...
const generateGameContent = async ({
  ...
  try {
    ...
    await adminCreateGenerationLog({
      ...
      action,
      prompt: rendered.userPrompt,
      result: templateKey === "game_article" ? normalizeGeneratedArticle(generated.text) : normalizeGeneratedMetaDescription(generated.text),
      status: "success"
    });
    return templateKey === "game_article" ? normalizeGeneratedArticle(generated.text) : normalizeGeneratedMetaDescription(generated.text);
...
async function adminGetCronPublishSettings() {
  if (!isConfigured() || !supabaseAdmin) {
    return { settings: DEFAULT_SETTINGS, error: getConfigError() };
  }
  const { data, error } = await supabaseAdmin.from("cron_publish_settings").select("key,value,value_type");
  if (error) {
    return { settings: DEFAULT_SETTINGS, error: error.message };
  }
  return { settings: settingsFromRows(data ?? []) };
}
async function adminUpdateCronPublishSettings(settings) {
  if (!isConfigured()) {
    return { success: false, error: getConfigError() };
  }
  const updates = [];
  ...
  if (settings.intervalLabel !== void 0) {
    updates.push({
  ...
  if (settings.requireArticle !== void 0) {
    updates.push({
  ...
  if (settings.requireMetaDescription !== void 0) {
    updates.push({
  ...
  if (settings.articleMaxTokens !== void 0) {
    updates.push({
  ...
  if (settings.metaDescriptionMaxTokens !== void 0) {
    updates.push({
  ...
  if (settings.categoryFilter !== void 0) {
    updates.push({
...
async function adminListInactiveGamesForCron({
  page,
  limit,
  search = "",
  categorySlug = "",
  ...
  let query = supabaseAdmin.from("games").select("id,title,slug,category_slug,thumbnail,iframe_url,seo_description,description,created_at", {
    count: "exact"
  }).eq("is_active", false).order("created_at", { ascending: true }).range(pagination.from, pagination.to);
  ...
  const total = count ?? 0;
  return {
    games: (data ?? []).map(mapInactiveGame),
    page: pagination.page,
    limit: pagination.limit,
    total,
...
async function adminGetCronPublishStats() {
  ...
  try {
    const [inactiveGamesCount, readyToPublishCount, missingArticleCount, missingMetaDescriptionCount] = await Promise.all([
...
async function adminGetCronPublishLogs({
  page,
  limit
} = {}) {
  const pagination = normalizePagination({ page, limit });
  ...
  const { data, error } = await supabaseAdmin.from("cron_publish_logs").select("*").order("created_at", { ascending: false }).range(pagination.from, pagination.to);
  if (error) {
    return {
      logs: [],
      page: pagination.page,
  ...
  const logs = (data ?? []).map(mapCronLog);
  return {
    logs,
    page: pagination.page,
    limit: pagination.limit,
...
async function runCronPublishOnce({
  source
}) {
  ...
  for (const game of games) {
    ...
    try {
      ...
      const { error: articleSaveError } = await supabaseAdmin.from("games").update({ description: article, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", game.id).eq("is_active", false);
      if (articleSaveError) {
      ...
      const { error: metaSaveError } = await supabaseAdmin.from("games").update({ seo_description: metaDescription, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", game.id).eq("is_active", false);
      if (metaSaveError) {
      ...
      const { error } = await supabaseAdmin.from("games").update({
        ...
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("id", game.id).eq("is_active", false);
      if (error) {
        throw new Error(error.message);
      }
      published += 1;
  ...
  await upsertSettings([
    { key: SETTINGS_KEYS.lastRunAt, value: (/* @__PURE__ */ new Date()).toISOString(), value_type: "datetime" },
    { key: SETTINGS_KEYS.todayDate, value: today, value_type: "date" },
    {
  ...
  return {
    ...
    skipped,
    runId,
    message: firstFailureMessage || void 0
  };
}

export { adminUpdateCronPublishSettings as a, adminGetCronPublishSettings as b, adminGetCronPublishStats as c, adminListInactiveGamesForCron as d, adminGetCronPublishLogs as e, runCronPublishOnce as r };
