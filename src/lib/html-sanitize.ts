...
const ALLOWED_SITE_CONTENT_TAGS = new Set([
  'h1',
  ...ALLOWED_TAGS,
]);

const ALLOWED_HOMEPAGE_CONTENT_TAGS = new Set([
  'h2',
  'h3',
  'p',
  'ul',
...
const isExternalHref = (href: string) => {
  try {
    ...
    return ['http:', 'https:'].includes(parsedHref.protocol)
      && parsedHref.origin !== siteUrl.origin;
  } catch {
    return false;
  }
};

const getHref = (attributes: string) => {
  const match = attributes.match(/\bhref\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+))/i);
  return match?.[1] ?? match?.[2] ?? match?.[3] ?? '';
};

export const sanitizeGameArticleHtml = (html: string) => {
  let sanitized = html;
  ...
  return sanitized.replace(/<\s*(\/?)\s*([a-zA-Z][a-zA-Z0-9-]*)\b([^<>]*)>/g, (
    _match,
    slash: string,
    rawTag: string,
    attributes: string,
    ...
    const externalAttributes = isExternalHref(href)
...
export const sanitizeSiteContentHtml = (html: string) => {
  let sanitized = html;
...
export const sanitizeHomepageContentHtml = (html: string) => {
  let sanitized = html;
